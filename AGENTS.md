...
- `packages/server` — Express + WebSocket + SQLite backend. Entry: `src/index.ts`. Uses `@meting/core` for multi-platform music search/playback (netease, tencent, kugou, kuwo, baidu) and `migu-music-api` for 咪咕. DB auto-creates `partysongs.db` in cwd.
...
- `packages/server/src/services/music.ts` — Multi-source search and URL resolution
